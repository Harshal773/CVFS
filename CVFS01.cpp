...
void DisplayHelp()
{
	...
	printf("closeall: To Display information of file using file descriptor\n");
...
{
	int i = 0;
	PINODE temp = head;          // first INODETABLE address.
...
int WriteFile(int fd, char *arr, int isize)
{
	...
	int limit=MAXFILESIZE-((UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize));
	if(limit>isize)
	{
	...
	{
		strncpy((UFDTArr[fd].ptrfiletable->ptrinode->Buffer)+(UFDTArr[fd].ptrfiletable->writeoffset),arr,limit);

        (UFDTArr[fd].ptrfiletable->writeoffset)=(UFDTArr[fd].ptrfiletable->writeoffset)+limit;

        (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)=(UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)+limit;
	}
...
int OpenFile(char *name,int mode)
{
	...
	temp = Get_Inode(name);    // address of inode table acquired by Get INODE(name) function.
	if(temp==NULL)
		return -2;
	...
	UFDTArr[i].ptrfiletable->count = 1;   // like reference count ie no of active current file.
	...
	UFDTArr[i].ptrfiletable->ptrinode = temp;         // address of inode table acquired by Get INODE(name) function.
...
{
	int i=0;
	PINODE temp = head;   // address of first inode table. 
...
{
	PINODE temp = head;       // head: address of first inode table.
	int i=0;
...
int main()
{
	...
	while(1)
	{
		fflush(stdin);              // clean previous garbage from keyboard.
		strcpy(str,"");               // make str string empty.

		printf("\nHarshal's VFS :>");

		fgets(str,80,stdin);           // get input string from keyboard.

		count=sscanf(str,"%s %s %s  %s",command[0],command[1],command[2],command[3]);  // tokenize input string into number of possible tokens.(token after each space).
		...
		else if(count==2)
		{
			...
			{
				ret = stat_file(command[1]);  // display information of file.
				if(ret==-1)
			...
			else if(strcmp(command[0],"close")==0)
			{
				...
				if(ret==-1)
					printf("ERROR: There is np such file\n");
				continue;
			}
			...
			else if(strcmp(command[0],"write")==0)
			{
				...
				ret = strlen(arr);   // strlen: count length of given string.
				if(ret==0)
				{
					printf("Error: Incorrect parameter\n");
					continue;
				}
				ret = WriteFile(fd,arr,ret);  //(FD,data,lengthof(data));
				if(ret==-1)
		...
		{
			if(strcmp(command[0],"create")==0)   // compares two strings character by character.
			{
			...
			{
				ret=OpenFile(command[1],atoi(command[2]));        // atoi: converts a character string to an integer value.
				if(ret>=0)
			...
			else if(strcmp(command[0],"read")==0)
			{
				...
				if(ret==-4)
					printf("ERROR: it not regurlar file\n");
				if(ret==0)
